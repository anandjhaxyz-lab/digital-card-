...
async function startServer() {
  ...
  app.get('/auth/me', requireAuth, (req, res) => {
    ...
    res.json({ id: user.id, email: user.email, name: user.user_metadata?.full_name, avatar: user.user_metadata?.avatar_url });
  });
  ...
  // ✅ BAAD MEIN generic route
  app.get('/api/profiles/:identifier', async (req, res) => {
    try {
      const { identifier } = req.params;
      let { data } = await supabaseAdmin.from('cards').select('*').eq('slug', identifier).single();
      if (!data) {
        ({ data } = await supabaseAdmin.from('cards').select('*').eq('id', identifier).single());
      }
  ...
  app.post('/api/profiles', requireAuth, async (req, res) => {
    ...
    try {
      ...
      if (existingCard) {
        const { data, error } = await supabaseAdmin.from('cards')
          .update({ slug, profile_data: profileData, updated_at: new Date().toISOString() })
          .eq('user_id', user.id).select().single();
        if (error) throw error;
        return res.json(data);
      } else {
        const { data, error } = await supabaseAdmin.from('cards')
          .insert({ user_id: user.id, slug, profile_data: { ...profileData, email: user.email } })
          .select().single();
        if (error) throw error;
        return res.json(data);
      }
