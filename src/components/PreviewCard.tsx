...
interface PreviewCardProps {
  ...
  onProfileUpdate?: (update: UserProfile | ((prev: UserProfile) => UserProfile)) => void;
}
...
export default function PreviewCard({ profile, isSharedView = false, onProfileUpdate }: PreviewCardProps) {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      console.log('Saving profile to Firestore from PreviewCard...', { id: profileId });
      ...
      // Update local profile with the ID and data from server
      if (onProfileUpdate) {
      ...
      if (profile.slug) {
        url.pathname = `/${profile.slug.trim().toLowerCase().replace(/[^a-z0-9-]/g, '-')}`;
      } else {
      ...
      alert('Failed to generate share link. Please check your connection.');
    } finally {
      setIsSaving(false);
    }
    return null;
